...
<style>
#ai-fab{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));inset-inline-start:16px;z-index:9990;width:52px;height:52px;border-radius:50%;background:#0B1D36;color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 8px 24px rgba(11,29,54,.28);transition:transform .25s cubic-bezier(.16,1,.3,1),box-shadow .25s;-webkit-tap-highlight-color:transparent}
#ai-fab:hover{transform:scale(1.08);box-shadow:0 12px 32px rgba(11,29,54,.32)}
#ai-fab:active{transform:scale(.92)}
#ai-fab .fab-ring{position:absolute;inset:-6px;border-radius:50%;border:2px solid rgba(232,93,4,.4);animation:ring 2.4s cubic-bezier(.65,.05,.36,1) infinite}
...
@media(max-width:767px){
  body.app-shell:not(.app-shell--no-tabs) #ai-fab{
    bottom:calc(64px + 16px + env(safe-area-inset-bottom,0px));
  }
  body.app-shell--no-tabs #ai-fab{
    bottom:calc(20px + env(safe-area-inset-bottom,0px));
  }
  #ai-panel{padding-bottom:env(safe-area-inset-bottom,0px)}
}
